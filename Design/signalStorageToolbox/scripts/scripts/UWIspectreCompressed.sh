...
echo only use UWIspectreCompressed.sh if you know what you are doing
echo typical scripts to use are UWIultrasim.sh or UWIspectre.sh

# UWIspectreCompressed.sh
# =======================
# run spectre with UWI compressed output
#
...
# - UWI_TO_HARDRIVE
# - UWI_ISTEP
# ---- a point of a current signal is stored if it has a larger difference
#      than UWI_ISTEP with the previous stored point
# - UWI_VSTEP
# ---- a point of a voltage signal is stored if it has a larger difference
#      than UWI_VSTEP with the previous stored point
...
if test "${UWI_ISTEP+set}" != set ; then 
  export UWI_ISTEP=0     # keep currents accurate
  #export UWI_ISTEP=1e-6
fi
if test "${UWI_VSTEP+set}" != set ; then 
  export UWI_VSTEP=0.05
fi
#export UWI_TSTEP=1000e-12
export UWI_TSTEP=50e-12  # don't bother
# ----------------------  }}}1

spectre -format uwi -uwifmt ${UWI_FORMAT} -uwilib ${UWI_LIB_PATH}/compressed_UWI${UWI_PLATFORM}.so $@
