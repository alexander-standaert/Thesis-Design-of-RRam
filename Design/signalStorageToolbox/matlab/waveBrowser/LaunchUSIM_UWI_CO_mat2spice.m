function Simspecs = LaunchUSIM_UWI_CO_mat2spice(Simspecs,sp,Parent)
%
...
ignore_list={'CMI-2318','SPECTRE-6006','CMI-2424','USIMDB-1224','USIMDB-1070'};


handle = @LaunchUSIM_UWI_CO_mat2spice;
...
try
    ...
    Tbuild = toc;
    if (Tbuild > 60)
       fprintf(1,'building the netlist required %g sec\n ',Tbuild) ;
    end
    ...
    %command = sprintf( '%s UWIultrasim.sh  %s.sp > out; %s',pre, name , post);
    command = sprintf( '%s runCO_UWIUltrasim.sh  %s.sp > out; %s',pre, name , post);
    system(command);
