function [ simulation tree ] = readUWINonCompressed( fileName, filter, exactNames, header )
% function [ simulation [tree] ] = readUWINonCompressed( fileName )
% function [ simulation [tree] ] = readUWINonCompressed( fileName, filter )
% function [ simulation [tree] ] = readUWINonCompressed( fileName, filter, exactNames )
% function [ simulation [tree] ] = readUWINonCompressed( fileName, filter, exactNames, header )
%
...
%    NCD  compressed Double
%    NCF  compressed Float
%    NCA  compressed Ascii
%    NCH  compressed halfprecision (hpf allows values from +-5.96046×10−8 to +-65504)
%         we scale by 1024
    ...
    try
        ...
        if( nargin<4 )
            %tic;
            simulation = SimulationNonCompressedUWI.read( fileName, filter, exactNames );
            %toc;
        else 
            %tic;
            simulation = SimulationNonCompressedUWI.readFromFileAndHeader( fileName, header, filter, exactNames ); % Header could be a string (the filename of the header) or a SimulationHeader
            %toc;
        end
            
    catch exception
        fprintf('ReadUWINonCompressed.m: can''t execute java binary \n');
        rethrow( exception );
    end
