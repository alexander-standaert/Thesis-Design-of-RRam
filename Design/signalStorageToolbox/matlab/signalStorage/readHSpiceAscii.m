function [ simulation tree ] = readHSpiceAscii( fileName, filter, exactNames )
% function [ simulation [tree] ] = readHSpiceAscii( fileName )
% function [ simulation [tree] ] = readHSpiceAscii( fileName, filter )
% function [ simulation [tree] ] = readHSpiceAscii( fileName, filter, exactNames )
%
...
%
    ...
    fprintf( 'WARNING: hspice ascii is deprecated use CSDF instead\n' );
   ...
   %% read the file
    try
        ...
        %tic;
        simulation = SimulationHSpiceAscii.read( fileName, filter, exactNames );
        %toc;
    catch exception
        fprintf('ReadHSpiceAscii.m: can''t execute java binary.  Likely causes: \n');
