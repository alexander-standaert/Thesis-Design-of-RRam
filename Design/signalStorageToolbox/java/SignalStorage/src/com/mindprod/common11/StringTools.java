 ...
 * @version 2.4 2009-04-30 - fix but in countLeading ( String text, String leads ).
 *          add pruneExcessBlankLines
 * @since 2003-05-15
 */
public class StringTools
     ...
     *
     * @param text   text with possible seading string, possibly empty, but not null.
     * @param toChop the leading string of interest.
     *
     ...
     * @see #chopTrailing(String,String)
     */
    public static String chopLeading( String text, String toChop )
        {
        if ( text.startsWith( toChop ) )
            {
     ...
     * @param toChop the trailing string of interest.
     *
     ...
     * @see #chopLeading(String,String)
     */
    public static String chopTrailing( String text, String toChop )
        {
        if ( text.endsWith( toChop ) )
            {
     ...
     * Does not collapse other whitespace.
     *
     ...
     *
     * @param text  text with possible leading characters, possibly empty, but not null.
     * @param leads the leading characters of interest, usually ' ' or '\n'
     *
     * @return count of leading matching characters, possibly 0.
     * @noinspection WeakerAccess
     */
    public static int countLeading( String text, String leads )
        {
        int count;
        for ( count = 0; count < text.length() && leads.indexOf( text.charAt( count ) ) >= 0; count++ )
            {
            // empty loop.
            }
        return count;
     ...
     *
     * @param text     text with possible trailing characters, possibly empty, but not null.
     * @param trailing the trailing characters of interest, usually ' ' or '\n'
     *
     * @return count of trailing matching characters, possibly 0.
     * @noinspection WeakerAccess
     */
    public static int countTrailing( String text, String trailing )
        {
        int length = text.length();
        // need defined outside the for loop.
        int count;
        for ( count = 0; count < length && trailing.indexOf( text.charAt( length - 1 - count ) ) >= 0; count++ )
            {
            // empty loop
            }
        return count;
     ...
     * Enhanced version available in com.mindprod.common11.StringTools for JDK 1.5+.
     *
     ...
     *
     * @return string with any of those trailing characters removed. ".com" would not only chop .com, but any combination of those letters e.g. mc.moc
     * @see #trimTrailing(String)
     * @see #chopTrailing(String,String)
     */
