 ...
 * <p/>
 * The 10 missing days in 1582 October.: <b>yes</b> (Pope Gregory's correction) 1582-10-05 to 1582-10-14 never
 * happened.
 * <p/>
 ...
 * @version 5.5 2009-05-05 - add isAnniversary
 * @since 1997-05-03
 */
public final class BigDate implements Cloneable, Serializable, Comparable
     ...
     * Different parts of the world made the transition at different times. Pope Gregory: 1582-10-04, British: 1752-09-02.
     */
    ...
    public static String getCopyright()
        {
        return "BigDate 5.1 freeware copyright (c) 1997-2009 Roedy Green, Canadian Mind Products, http://mindprod.com roedyg@mindprod.com";
        }
     ...
     * <p/>
     * February 29 is treated specially, to handle the way people people born on Feb 29 celebrate their birth days on Feb 28
     ...
     * @param asOf      usually today, the day you want the to know if it is an anniversary. asof must come after birthDate to get a
     *                  meaningful result.
     *
     ...
     * Canada) adopted it in 1752. By then, 11 days had to be dropped. 1752-09-02 was followed immediately by 1752-09-14.
    ...
    public static boolean isValid( String yyyy_mm_dd )
        {
        final int[] ymd = parse( yyyy_mm_dd );
        if ( ymd == null )
            {
            return false;
            }
        return BigDate.isValid( ymd[ 0 ], ymd[ 1 ], ymd[ 2 ] );
        }
     ...
     * Also handles AD/BC, 1-digit months, 1-digit days and 1, 2 or 3-digit years.  Date must be a valid normalised date.
     *
    ...
    public void setDateAtTime( long utcTimestamp, TimeZone timeZone )
        ...
        */
        int offsetInMillis =
                timeZone.getOffset( getOrdinal() >= Jan_01_0001 ? 1 : 0,
                        getYYYY(),
                        getMM() - 1,
                        getDD(),
                        getCalendarDayOfWeek(),
    ...
    public String toDowMMDDYY()
        ...
        return dayAbbr[ getDayOfWeek() ]
               + " "
               + StringTools.toLZ( mm, 2 )
               + "/"
               + StringTools.toLZ( dd, 2 )
               + "/"
    ...
    static
        ...
        */
        ...
        GC_firstOrdinal =
                BigDate.toOrdinal( GC_firstYYYY, GC_firstMM, GC_firstDD );
     ...
     *
     * @return array[] yyyy, mm, dd, with yyyy negative for BC. null for unparsable date.
     */
    private static int[] parse( String yyyy_mm_dd )
        {
        try
            {
    ...
    protected final void toGregorian()
        ...
        if ( ordinal > MAX_ORDINAL )
            {
            throw new IllegalArgumentException( "invalid ordinal date: "
                                                + ordinal );
            }
        else if ( ordinal >= GC_firstOrdinal )
            {
            yyyy =
                    Leap400RuleYYYY + flooredMulDiv( ordinal
                                                     - Jan_01_Leap400RuleYear,
                            10000,
