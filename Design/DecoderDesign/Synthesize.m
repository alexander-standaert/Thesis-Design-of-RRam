function [Emean,extreme_E,delaymean,extreme_td] = Synthesize(NoI,whichdecoder)
sp.NoI=NoI;
sp.NoISTRING = int2spelledstring(NoI);

if whichdecoder == 1
    sp.whichdecoder = '';
elseif whichdecoder == 2
    sp.whichdecoder = '2';
end
...
for i=1:sp.numruns
    ...
    delay(i)=t(tstart+min(find(v(tstart:end)>0.98))-1)-1e-9;
    
    [~, t2start] = min(abs(t2 - 1e-9));
    [~, t2stop] = min(abs(t2 - (1e-9+delay(i))));
