function [ simulation tree ] = readPsfAscii( fileName, filter, exactNames )
% function [ simulation [tree] ] = readPsfAscii( fileName )
% function [ simulation [tree] ] = readPsfAscii( fileName, filter )
% function [ simulation [tree] ] = readPsfAscii( fileName, filter, exactNames )
%
% <simulation> will be a java object of class
% be.kuleuven.micas.signalStorage.interfaces.LoadableSignalDatabase
%
%   <fileName> is the name of the PsfAscii file to be loaded.
% 
...
%
   ...
   %% read the file
    try
        ...
        %tic;
        simulation = SimulationPsfAscii.read( fileName, filter, exactNames );
        %toc;
    catch exception
        fprintf('ReadPsfAscii.m: can''t execute java binary \n');
        rethrow( exception );
    end
