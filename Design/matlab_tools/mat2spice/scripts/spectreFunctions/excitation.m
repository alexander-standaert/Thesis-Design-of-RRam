function [ netlist ] = ...
  excitation( name, numberOfBits, values, low, high, period, risetime )
%function [ netlist ] = ...
%  excitation( name, indices, values, low, high, period, risetime )
%
% generates a PWL for each name_index 
...
for i=1:numberOfBits
  ...
  time0 = 0;
  v0 = getVoltage( values(1), bitIndex, low, high );
  ...
  for j=2:length(values)
    ...
    v0    = getVoltage( values(j), bitIndex, low, high );
    time1 = (period)*(j-1);
    v1    = getVoltage( values(j)  , bitIndex, low, high );
...
function [ v ] = getVoltage( value, bitIndex, low, high )
