function [ ] = semilogyGraph( plotS, plotId )
% function [ ] = semilgyGraph( plotS, plotId )
%
...
for i=1:length( plotS.name )
  ...
  if( plotS.lineWidth{i} > 0 )
    semilogy( xv*xm, yv*ym, style, 'lineWidth', plotS.lineWidth{i}, 'markerSize', 9  );
  else
    semilogy( xv*xm, yv*ym, style, 'markerSize', 9 );
  end 
  hold on;
...
for i=1:length( plotS.name )
    ...
    if( plotS.lineWidth{i} > 0 )
      semilogy( xv*xm, yv*ym, style, 'lineWidth', plotS.lineWidth{i}  );
    else
        semilogy( xv*xm, yv*ym, style );
    end 
end
